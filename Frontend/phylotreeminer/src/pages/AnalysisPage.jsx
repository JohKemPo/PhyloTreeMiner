...
const AnalysisPage = () => {
  ...
  const processFile = (file) => {
    ...
    reader.onload = (e) => {
      ...
      if (["nwk", "newick", "nexus", "tree", "tre"].includes(extension)) {
        ...
        type = "alignment";
      } else if (["cql"].includes(extension)) {
        type = "cql";
      } else {
  ...
  const draggerProps = {
    name: "file",
    multiple: false,
    accept: ".nexus,.nwk,.newick,.tree,.fasta,.fa,.fas,.aln,.clustal,.cql",
    beforeUpload: (file) => {
      processFile(file);
      return false;
    },
      ...
      return (
        <Dragger {...draggerProps} style={{ padding: "20px" }}>
          ...
          <p className="ant-upload-hint">
            Support for tree (.nexus, .nwk) and alignment (.fasta, .aln) and CQL files.
          </p>
        </Dragger>
      );
    }
  ...
  const tabItems = [
    {
      ...
      label: (
        <>
          <ExperimentOutlined /> Phylogenetic Analysis
        </>
      ),
      children: (
        <Row gutter={[16, 16]}>
    ...
    },
    {
      ...
      label: (
        <>
          <ShareAltOutlined /> Graph Visualization
        </>
      ),
  ...
  return (
    <Layout>
      <Content
        style={{ padding: "24px", backgroundColor: "#ffffff", borderRadius: 8 }}
      >
