...
> **Estado:** ⚠️ **aberto** (itens 2-4 da correção continuam pendentes; artefatos em disco continuam com a composição antiga). Descoberto em 2026-08-26 ([DEC-050](../automation/07-log-de-execucao.md)) ao remedir os alinhadores para a [decisão 1](../automation/08-ficha-de-fatos.md). Quem o denunciou foi o **MUSCLE 5.3**, com um `WARNING: 1 duplicate labels` que nenhuma outra ferramenta emitia. **A pergunta "qual registro sobrevive quando GenBank e RefSeq coexistem" foi decidida em 2026-09-04** ([DEC-082](../automation/07-log-de-execucao.md#dec-082--2026-09-04--d23--decisão-do-usuário-entre-as-opções-a-d-a-preferir-refseq-relabelando-na-posição-de-hoje)): **opção A** — preferir RefSeq, relabelando na posição de hoje. `efetch` nos três pares confirmou `source` idêntico (mesmo `geo_loc_name`/`collection_date`, quando presentes) — preferir RefSeq não perde metadado filogeográfico em nenhum dos três casos. **O item 1 da correção abaixo está implementado** ([DEC-083](../automation/07-log-de-execucao.md#dec-083--2026-09-04--d23--item-1-implementado-aquisição-e-a-rota-de-gerar-dataset-via-genbank-passam-a-preferir-refseq-na-posição-de-hoje), nas duas rotas de aquisição — `workflow_dataAcquisition.py` e a rota de gerar dataset via GenBank da UI/backend); a terceira implementação do mesmo defeito, `dataValidation.deduplicar_por_sequencia` (rota do `TreeBuilderController`, item 2, já renomeada e logada desde DEC-050), **não recebeu a preferência** — fora do escopo pedido. Nenhum artefato em disco muda: só reaquisição materializa.
...
| # | O quê | Onde |
|---|---|---|
| 1 | A aquisição não baixa RefSeq e GenBank do mesmo genoma: **decidido (DEC-082) e implementado (DEC-083)** — preferir RefSeq, relabelando na posição em que a deduplicação já mantém o sobrevivente hoje. Cobre as duas rotas de aquisição (`workflow_dataAcquisition.py` e a rota de gerar dataset via GenBank da UI/backend); a dedup do `TreeBuilderController` (item 2) segue sem a preferência | `workflow_dataAcquisition.py`, `Backend/src/services/ncbi_acquisition.py` — é o [M2.1](../automation/10-marcos-e-metas.md) |
