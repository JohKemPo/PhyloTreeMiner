...
| M4.4 | ✅ `ADMIN_TOKEN` em `/api/ncbi/set-email` e `/neo4j/connect` ([DEC-065](07-log-de-execucao.md)); comparação corrigida para `secrets.compare_digest` — B3 da revisão ([DEC-066](07-log-de-execucao.md)) | `S-5`/DEC-004 | `seguranca.py` (novo), `app.py`, `neo4j_router.py`, `tests/api/test_admin_token.py` (novo) | sem header → `401`; token correto → passa — 7 passed | M4.2 |
...
| M4.6 | ✅ Limites rígidos: bytes/nº de arquivos em `/upload-data`, razão de expansão do ZIP, teto de `retmax` ([DEC-065](07-log-de-execucao.md)); teto de bytes passou a ser aplicado durante a leitura (não depois) e a defesa de zip bomb passou a medir o byte real descomprimido, não `ZipInfo.file_size` (spoofável) — B1/B2 da revisão ([DEC-066](07-log-de-execucao.md)) | `S-5` | `app.py`, `tests/api/test_limites_entrada.py`, `tests/unit/test_upload_seguranca.py` (novo) | acima do teto → `413`/`400`/`422` — 6+4 passed | M4.5 |
...
| M4.10 | ✅ `get_metadata_cache`/`Phylo.convert`/comparação de árvores saem do loop ([DEC-065](07-log-de-execucao.md)). **`render_annotated_tree` (ete3/PyQt) NÃO** — mover para `asyncio.to_thread` causava `SIGSEGV` reproduzido (B4 da revisão); revertido para síncrono, só a leitura de metadata do `gen_plot` ficou em thread ([DEC-066](07-log-de-execucao.md)) | `B-5` | `app.py`, `tests/api/test_cpu_bound_to_thread.py` (guarda AST contra reintroduzir o `to_thread` em `render_annotated_tree`) | refatoração pura de transporte; suíte completa sem regressão — 5 passed no teste dedicado | M4.9 |
