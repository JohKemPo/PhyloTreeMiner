...
| 2026-09-13 | `SubtreeMinerController` (`workflow/controller/subtreeMinerController.py`) é **código morto e quebrado**: nada em produção o instancia (`workflow.py` usa só `TreeBuilderController`/`SubtreeBuilderController`); `subtreeBuilderController.py` importa o nome `SubtreeMiner` do mesmo módulo, mas por reexportação transitiva pega a classe real de `subtree_mining/miner.py`, não `SubtreeMinerController`. Se alguém instanciasse `SubtreeMinerController`, os dois ramos do seu `miner()` quebrariam: `SubtreeMiner.group_data_by_tree_base(data)` chamado sem instância (TypeError, `data` vira `self`) e `self.process_group(data)` num método que a classe nunca definiu (AttributeError) | `BioComp_UFF/workflow/controller/subtreeMinerController.py` (arquivo inteiro, 130 linhas) | Avaliação de Arq-D (`docs/automation/14-avaliacao-arq-d-biocomp-uff.md`) | **Candidato a exclusão** — baixo risco (código inalcançável, não muda comportamento), mas fora do pedido da avaliação (que era só documentar); lote curto e independente de qualquer decisão sobre reestruturar `treeBuilderController.py` |
